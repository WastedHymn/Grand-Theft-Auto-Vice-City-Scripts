...
CFLAGS	:=	-g -Wl,-q -fno-optimize-sibling-calls -O2 -ftree-vectorize -mfloat-abi=hard $(ARCH) $(DEFINES)
CFLAGS	+=	$(INCLUDE) -DPSP2 -DLIBRW -DRW_GL3 -DAUDIO_OAL -DLIBRW_GLAD
...
LIBS	:=  -lrw -lopenal -lSDL2 -lvita2d -lvitagl -lSceAppMgr_stub -lSceDisplay_stub -lSceCommonDialog_stub -lSceLibKernel_stub \
			...
			-lSceCtrl_stub -lSceHid_stub -lSceAudio_stub -lSceTouch_stub -lm -lpthread -lmpg123 -lsndfile

all: $(TARGET).vpk

$(TARGET).vpk: $(TARGET).velf
	vita-make-fself -c -s $< build/eboot.bin
	vita-mksfoex -s TITLE_ID=$(TITLE) -d ATTRIBUTE2=12 "Grand Theft Auto: Vice City" param.sfo
